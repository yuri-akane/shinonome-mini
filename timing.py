...
class BpmTimeline:
    ...
    def __init__(self, initial_bpm: float, bpm_events: List[Tuple[float, float]], 
        ...
        scroll_events: list of (beat, scroll_multiplier).
        """
        ...
        self.scroll_events = sorted(scroll_events, key=lambda x: x[0]) if scroll_events else []
    ...
    def _build(self):
        ...
        for beat, _ in self.stop_events:
            transition_beats.add(beat)
        for beat, _ in self.scroll_events:
            transition_beats.add(beat)
        ...
        scroll_map = dict(self.scroll_events)
        ...
        for i in range(len(sorted_beats) - 1):
            ...
            # Update active SCROLL multiplier if there is a change at b_start
            if b_start in scroll_map:
                curr_scroll = scroll_map[b_start]
                
            mult = self._get_measure_multiplier(b_start)
            
            delta_beat = b_end - b_start
            delta_time = delta_beat * (60.0 / curr_bpm)
            delta_height = delta_beat * curr_scroll
            ...
            if b_end in stop_map:
                ...
                if b_end in scroll_map:
                    curr_scroll = scroll_map[b_end]
        ...
        if last_beat in scroll_map:
            curr_scroll = scroll_map[last_beat]
        ...
        final_seg = TimelineSegment(
            ...
            start_height=curr_height,
            end_height=curr_height + 1e6 * curr_scroll,
            bpm=curr_bpm,
            multiplier=mult,
            scroll_factor=curr_scroll,
            is_stop=False
